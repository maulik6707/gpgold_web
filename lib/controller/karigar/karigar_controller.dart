...
class KarigarController extends GetxController {
  ...
  Future<dynamic> deleteKarigar(int id) async {

    try {
      isLoading(true);

      print("Delete karigar url ${API.addKarigar}&id=$id");

      final response = await http.delete(Uri.parse("${API.addKarigar}&id=$id"),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
          'Authorization': await SharPreferences.getString('AccessToken') ?? ""
        },);
      ...
      print("Delete karigar response ${responseBody}");
